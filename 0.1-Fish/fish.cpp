...
#include <stylizer/model/thirdparty/tinyobjloader.h>
...
#include <stylizer/model/load.hpp>
...
int main() {
	...
	auto fb = stylizer::color_depth_frame_buffer::create(
		ctx, {800, 600, 1}, std::math::color32::from_hex("#FFF9E5"), window.texture_format()
	);
	...
	util.camera.projection = std::math::perspective_matrix<float>(std::math::degree(45), fb.size().xy().to_vector());
	util.camera.inverse_projection = inverse(util.camera.projection);
	util.camera.view = std::math::look_at_matrix({0, 0, 2}, {0, 0, 0});
	...
	while(!window.close_requested()) {
		...
		instances[0].model = make_homogenous(rotation_matrix(std::math::quaternion::from_euler_angles({0, util.time.total * 90, 0})));
		instances.upload(ctx);

		fb.draw_to(ctx, [&](stylizer::graphics::current_backend::render::pass& pass) {
